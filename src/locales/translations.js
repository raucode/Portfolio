import { Home } from "lucide-react";

export const translations = {
  en: {
    nav: {
      home: "Home",
      about: "About",
      projects: "Projects",
      stack: "Stack",
      contact: "Contact",
      cta: "Let’s talk",
    },
    hero: {
        quote: "(Never stop learning)",
        compactTitle: "Systems, product thinking and clean architecture.",
        profileName: "Raúl Figuera",
        profileRole: "Backend-first developer • Product-minded • Full stack",
        description:
            "Full stack developer focused on backend architecture, product thinking and scalable foundations. I build software that is meant to grow, not just to work.",
        primaryCta: "View projects",
        secondaryCta: "Contact me",
        panel: {
            label: "System thinking",
            title: "Architecture first",
            description:
            "Focused on modular systems, clear boundaries and real product logic.",
            items: [
            {
                label: "Auth",
                title: "Role inheritance"
            },
            {
                label: "Billing",
                title: "Subscriptions & payments"
            },
            {
                label: "Frontend",
                title: "Product-oriented UI"
            },
            {
                label: "Tooling",
                title: "Linux environment design"
            }
            ]
        }
    },
    projects: {
      eyebrow: "Projects",
    },
    stack: {
      eyebro: "Stacks"
    },
    footer: {
      text: "Built with React."
    }
  },

  es: {
    nav: {
      home: "Inicio",
      about: "Sobre mí",
      projects: "Proyectos",
      stack: "Stack",
      contact: "Contacto",
      cta: "Hablemos",
    },
    hero: {
        quote: "(Nunca pares de aprender)",
        compactTitle: "Sistemas, pensamiento de producto y arquitectura limpia.",
        profileName: "Raúl Figuera",
        profileRole: "Backend-first developer • Full stack",
        description:
            "Desarrollador full stack con foco en arquitectura backend, visión de producto y bases escalables. Construyo software pensado para crecer, no solo para funcionar.",
        primaryCta: "Ver proyectos",
        secondaryCta: "Contactarme",
        panel: {
            label: "Pensamiento sistémico",
            title: "Arquitectura primero",
            description:
            "Enfoque en sistemas modulares, límites claros y lógica real de producto.",
            items: [
            {
                label: "Planeacion",
                title: "Diagramacion, gestion de datos"
            },
            {
                label: "Backend",
                title: "Estructura, endpoints y datos"
            },
            {
                label: "Frontend",
                title: "UI orientada a producto y UX"
            },
            {
                label: "Tooling",
                title: "Diseño de entorno desktop"
            }
            ]
        }
    },
    
    projects: {
      eyebrow: "Proyectos",
    },
        stack: {
      eyebro: "Stacks"
    },
    footer: {
      text: "Hecho con React."
    }
  },

  pt: {
    nav: {
      home: "Inicio",
      about: "Sobre mim",
      projects: "Projetos",
      stack: "Stack",
      contact: "Contato",
      cta: "Vamos conversar",
    },
    hero: {
        quote: "(Nunca pare de aprender)",
        compactTitle: "Sistemas, pensamento de produto e arquitetura limpa.",
        profileName: "Raúl Figuera",
        profileRole: "Backend-first developer • Product-minded • Full stack",
        description:
            "Desenvolvedor full stack com foco em arquitetura backend, visão de produto e bases escaláveis. Construo software pensado para crescer, não apenas para funcionar.",
        primaryCta: "Ver projetos",
        secondaryCta: "Entrar em contato",
        panel: {
            label: "Pensamento sistêmico",
            title: "Arquitetura primeiro",
            description:
            "Foco em sistemas modulares, limites claros e lógica real de produto.",
            items: [
            {
                label: "Auth",
                title: "Papéis hereditários"
            },
            {
                label: "Pagamentos",
                title: "Assinaturas e cobrança"
            },
            {
                label: "Frontend",
                title: "UI orientada a produto"
            },
            {
                label: "Tooling",
                title: "Design de ambiente Linux"
            }
            ]
        }
    },    
    projects: {
      eyebrow: "Projetos",
    },
        stack: {
      eyebro: "Stacks"
    },
    footer: {
      text: "Feito com React."
    }
  }
};